...
function writeJoint(name: string, level: number, lines: string[]) {
  ...
  lines.push(`${indent(level)}${keyword} ${joint.name}`);
  ...
  if (joint.parent == null) {
    lines.push(
      `${indent(level + 1)}CHANNELS 6 Xposition Yposition Zposition Xrotation Yrotation Zrotation`,
    );
  } else {
    lines.push(`${indent(level + 1)}CHANNELS 3 Xrotation Yrotation Zrotation`);
  }
...
export function writeBvh(motion: SolvedMotionArtifact) {
  ...
  for (const frame of motion.frames) {
    const values: number[] = [
      ...
      ...(frame.joints.Hips ?? [0, 0, 0]),
    ];
    for (const joint of SKELETON.slice(1)) {
      values.push(...(frame.joints[joint.name] ?? [0, 0, 0]));
    }
